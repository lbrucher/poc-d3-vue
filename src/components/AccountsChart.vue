...
function createChart() {
  svg = d3.select(chartElement.value).append('svg:svg').attr('width', '100%').attr('height', '100%')
...
function drawBars() {
  ...
  const t = d3.transition().duration(750)
  const bars = svg.selectAll('rect.bar').data(props.accounts, (a) => a.id)
...
<template>
  <div class="account-chart-container">
    <div ref="chartElement" class="account-chart" @click="$emit('click')">
      <div v-if="debug">
        <div v-for="acc in accounts" :key="acc.id" style="font-size: 80%">
